...
export class MailChimpMailerService {
  ...
  onCandidateSignSuccess(user: CandidateDetail) {
    console.log(user);
    let listId = config.get('application.mail.MAILCHIMP_CANDIDATE_SIGN_IN_SUCCESS_LISTID');
    this.triggerMailChimpService(user, listId);
  }

  onCandidatePofileSubmitted(user:CandidateDetail) {
      let listId = config.get('application.mail.MAILCHIMP_CANDIDATE_PROFILE_SUBMIT_SUCCESS_LISTID');
      this.triggerMailChimpService(user, listId);
  }
  ...
  triggerMailChimpService(data: CandidateDetail, listId: string) {
    if (data !== null && data !== undefined) {
      ...
      let mailchimpClient = new mailchimp(config.get('application.mail.MAIL_CHIMP_SERVICE_KEY'));
      let md5 = require('md5');
